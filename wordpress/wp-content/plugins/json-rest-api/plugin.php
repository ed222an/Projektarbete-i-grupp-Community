...
function json_api_default_filters( $server ) {
	...
	// Achievemets.
	$wp_json_achievemets = new WP_JSON_Achievemets( $server );
	add_filter( 'json_endpoints',       array( $wp_json_achievemets, 'register_routes'         ), 0     );
	add_filter( 'json_prepare_post',    array( $wp_json_achievemets, 'add_post_author_data'    ), 10, 3 );
	add_filter( 'json_prepare_comment', array( $wp_json_achievemets, 'add_comment_author_data' ), 10, 3 );
