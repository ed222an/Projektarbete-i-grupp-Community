...
function vscf_shortcode($vscf_atts) {
	...
	if (($_SERVER['REQUEST_METHOD'] == 'POST') && isset($_POST['form_send']) ) {
		...
		if ($error == false) {
			...
			wp_mail($vscf_atts['email_to'], $email_subject, $email_message, $headers);
			$result = $vscf_atts['success'];
			$sent = true;
		}
	}
	...
	$email_form = '<form class="vscf" id="vscf" method="post" action="">
		...
		<p><label for="vscf_name">'.$vscf_atts['label_name'].': <span class="'.((isset($error_class['form_name'])) ? "error" : "hide").'" >'.$vscf_atts['error_form_name'].'</span></label></p>
		...
		<p><label for="vscf_email">'.$vscf_atts['label_email'].': <span class="'.((isset($error_class['email'])) ? "error" : "hide").'" >'.$vscf_atts['error_email'].'</span></label></p>
		<p><input type="text" name="email" id="vscf_email" class="'.((isset($error_class['email'])) ? "error" : "").'" maxlength="50" value="'.$form_data['email'].'" /></p>
		
		<p><label for="vscf_subject">'.$vscf_atts['label_subject'].': <span class="'.((isset($error_class['form_subject'])) ? "error" : "hide").'" >'.$vscf_atts['error_form_subject'].'</span></label></p>
		...
		<p><label for="vscf_sum">'.$vscf_atts['label_sum'].' '. $_SESSION['vscf-rand'].': <span class="'.((isset($error_class['form_sum'])) ? "error" : "hide").'" >'.$vscf_atts['error_form_sum'].'</span></label></p>
		...
		<p><label for="vscf_message">'.$vscf_atts['label_message'].': <span class="'.((isset($error_class['form_message'])) ? "error" : "hide").'" >'.$vscf_atts['error_form_message'].'</span></label></p>
		...
		<p><input type="submit" value="'.$vscf_atts['label_submit'].'" name="form_send" class="vscf_send" id="vscf_send" /></p>
